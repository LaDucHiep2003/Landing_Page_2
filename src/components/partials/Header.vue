<template>
  <div class="bg-white border-b border-color-2 w-[calc(100%-19rem)] ml-auto max-lg:w-full" :class="{ 'w-full' : !openSidebar}">
    <div class="p-4 flex justify-between items-center h-[70px]">
      <div class="flex items-center gap-2">
        <div :class="{ hidden: $route.name === 'Home' || $route.name === 'quanlynguoidung' || $route.name === 'danhsachcongviec' || $route.name === 'phan-nhom-nguoi-dung'}" @click="goBack" class="w-6 h-6 bg-white rounded-[4px] flex justify-center items-center cursor-pointer border border-color-2">
          <svg class="rotate-[270deg]" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M15 12.5L10 7.5L5 12.5" stroke="#667085" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <div class="text-xl font-semibold text-color-1 max-sm:text-lg">{{ pageTitle }}</div>
      </div>
      <div class="h-10 flex justify-between items-center gap-5 max-md:h-6 max-md:gap-2">
        <div class="bg-linear-gradient-1 px-4 h-10 rounded-md flex justify-center items-center gap-1 max-md:h-6 max-md:px-1">
          <svg class="max-lg:h-4 max-md:w-4" width="20" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M2.1665 10.0126C2.1665 5.62268 5.67484 1.66675 10.5165 1.66675C15.2498 1.66675 18.8332 5.54757 18.8332 9.98756C18.8332 15.137 14.6332 18.3334 10.4998 18.3334C9.13317 18.3334 7.6165 17.9662 6.39984 17.2485C5.97484 16.9897 5.6165 16.7978 5.15817 16.948L3.47484 17.4488C3.04984 17.5823 2.6665 17.2485 2.7915 16.7978L3.34984 14.9283C3.4415 14.6696 3.42484 14.3942 3.2915 14.1772C2.57484 12.8585 2.1665 11.4147 2.1665 10.0126ZM9.4165 10.0126C9.4165 10.6052 9.8915 11.0809 10.4832 11.0892C11.0748 11.0892 11.5498 10.6052 11.5498 10.0209C11.5498 9.42839 11.0748 8.95268 10.4832 8.95268C9.89984 8.94433 9.4165 9.42839 9.4165 10.0126ZM13.2582 10.0209C13.2582 10.6052 13.7332 11.0892 14.3248 11.0892C14.9165 11.0892 15.3915 10.6052 15.3915 10.0209C15.3915 9.42839 14.9165 8.95268 14.3248 8.95268C13.7332 8.95268 13.2582 9.42839 13.2582 10.0209ZM6.6415 11.0892C6.05817 11.0892 5.57484 10.6052 5.57484 10.0209C5.57484 9.42839 6.04984 8.95268 6.6415 8.95268C7.23317 8.95268 7.70817 9.42839 7.70817 10.0209C7.70817 10.6052 7.23317 11.0809 6.6415 11.0892Z" fill="white"/>
          </svg>
          <div class="text-sm font-semibold text-white max-md:hidden">Hỏi chat bot</div>
        </div>
        <div>
          <svg class="max-md:h-4 max-md:w-4" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_816_35419)">
              <path d="M12.0054 0.00244141C5.37818 0.00244141 0.00537109 5.37525 0.00537109 12.0024C0.00537109 18.6296 5.37818 24.0024 12.0054 24.0024C18.6326 24.0024 24.0054 18.6301 24.0054 12.0024C24.0054 5.37478 18.633 0.00244141 12.0054 0.00244141ZM21.491 10.7837H17.8554C17.7193 8.94653 17.2627 7.14735 16.5063 5.4676C16.1104 4.58553 15.6352 3.74128 15.0865 2.94525C16.7792 3.52381 18.2762 4.56462 19.4081 5.9499C20.5399 7.33517 21.2615 9.0096 21.491 10.7837ZM12.0054 21.0357C10.0548 18.868 8.86049 16.1258 8.60178 13.2212H15.409C15.2784 14.7131 14.8981 16.1725 14.284 17.5384C13.7111 18.8157 12.9423 19.9957 12.0054 21.0357ZM8.60178 10.7837C8.86049 7.87918 10.0547 5.13703 12.0049 2.96916C12.9419 4.01 13.7105 5.19079 14.283 6.46885C14.8969 7.83485 15.2772 9.29417 15.408 10.786L8.60178 10.7837ZM5.24365 5.24072C6.27735 4.20219 7.53674 3.41618 8.92381 2.94385C8.38142 3.73055 7.91078 4.56436 7.51756 5.43525C6.75285 7.12361 6.29143 8.93343 6.15443 10.7818H2.51975C2.78473 8.68281 3.74346 6.73252 5.24365 5.24072ZM2.51975 13.2212H6.15537C6.29164 15.0584 6.74837 16.8576 7.5049 18.5373C7.90078 19.4193 8.37602 20.2636 8.92475 21.0596C7.23186 20.4812 5.7347 19.4405 4.60274 18.0552C3.47078 16.6699 2.74922 14.9954 2.51975 13.2212ZM18.7671 18.7642C17.7332 19.8029 16.4734 20.5889 15.086 21.061C15.635 20.2651 16.1105 19.4209 16.5063 18.5387C17.2627 16.8589 17.7193 15.0598 17.8554 13.2226H21.491C21.2262 15.3218 20.2675 17.2723 18.7671 18.7642Z" fill="#F7C92B"/>
              <path d="M9.7282 17.5384C9.11408 16.1725 8.73379 14.7131 8.6032 13.2212H15.409C15.3676 13.6885 15.3018 14.1534 15.2121 14.6139C16.3417 13.3975 17.2286 11.9766 17.8249 10.4275C17.6586 8.71337 17.2132 7.038 16.5063 5.46761C16.1104 4.58555 15.6352 3.74129 15.0865 2.94527C16.433 3.40373 17.6598 4.15798 18.6766 5.15261C18.6229 3.94945 18.402 2.75961 18.0204 1.6173C16.1935 0.55644 14.1179 -0.000794129 12.0054 0.00245554C5.3782 0.00245554 0.00538769 5.37527 0.00538769 12.0025C0.00175519 14.1819 0.594394 16.3208 1.71914 18.1876C2.86712 18.5495 4.0597 18.7507 5.26289 18.7853L5.24367 18.7665C3.74255 17.2737 2.78374 15.3217 2.51976 13.2212H6.15539C6.29166 15.0584 6.74839 16.8576 7.50492 18.5373C7.52179 18.5753 7.53961 18.6123 7.55648 18.6498C8.37936 18.5292 9.18872 18.3297 9.97336 18.054C9.88679 17.8843 9.80507 17.7125 9.7282 17.5384ZM6.15539 10.7837H2.51976C2.74919 9.0097 3.47062 7.33533 4.60241 5.95012C5.7342 4.56492 7.23115 3.52419 8.92383 2.94574C8.38144 3.73244 7.9108 4.56625 7.51758 5.43714C6.75319 7.12555 6.29209 8.93538 6.15539 10.7837ZM9.73992 6.44027C10.3113 5.17401 11.0754 4.00389 12.0049 2.97152C12.9417 4.01164 13.7103 5.19163 14.283 6.46886C14.8969 7.83486 15.2772 9.29419 15.408 10.786H8.60179C8.73346 9.28365 9.11826 7.81434 9.73992 6.44027Z" fill="#FFDF55"/>
            </g>
            <defs>
              <clipPath id="clip0_816_35419">
                <rect width="24" height="24" fill="white"/>
              </clipPath>
            </defs>
          </svg>
        </div>
        <div>
          <svg class="max-md:h-4 max-md:w-4" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_816_35420)">
              <path d="M14.8419 21.0042C14.8419 19.3495 13.4984 18.0083 11.8411 18.0083C10.1838 18.0083 8.84033 19.3495 8.84033 21.0041C8.84033 22.6587 10.1838 24 11.8411 24C13.4985 24 14.8419 22.6587 14.8419 21.0042Z" fill="#F7C92B"/>
              <path d="M11.8434 18.0083C11.54 18.0083 11.2472 18.0536 10.9712 18.1373C12.203 18.5102 13.0998 19.6523 13.0998 21.0042C13.0998 22.3558 12.203 23.4981 10.9712 23.871C11.254 23.9567 11.5479 24.0002 11.8434 24C13.5007 24 14.8441 22.6587 14.8441 21.0042C14.8441 19.3496 13.5007 18.0083 11.8434 18.0083Z" fill="#F2B51D"/>
              <path d="M12.1933 3.51934C11.2295 3.71259 10.2914 3.08951 10.0977 2.12722C9.90404 1.16513 10.5284 0.228509 11.492 0.0351604C12.4556 -0.158138 13.3938 0.464991 13.5876 1.42723C13.781 2.38932 13.1569 3.32599 12.1933 3.51934Z" fill="#F7C92B"/>
              <path d="M13.5892 1.42723C13.3956 0.464991 12.4573 -0.158138 11.4936 0.0351604C11.4039 0.0531742 11.3174 0.0777931 11.2339 0.107866C11.7964 0.313525 12.2419 0.797898 12.3686 1.42728C12.5442 2.2998 12.047 3.15146 11.2341 3.44659C11.5415 3.55899 11.8741 3.58416 12.195 3.51929C13.1586 3.32599 13.7827 2.38927 13.5892 1.42723Z" fill="#F2B51D"/>
              <path d="M19.2748 20.5028H4.41031C3.08013 20.5028 2.00195 19.4263 2.00195 18.0985C2.00204 17.7354 2.0835 17.377 2.24035 17.0495C2.39719 16.722 2.62544 16.4339 2.9083 16.2062L3.38582 15.8216C4.49037 14.9322 5.13266 13.5915 5.13266 12.1748V9.6481C5.13266 5.92003 8.15965 2.89795 11.8938 2.89795C15.6276 2.89795 18.6548 5.92003 18.6548 9.6481V12.2216C18.655 12.9084 18.8062 13.5869 19.0979 14.2087C19.3895 14.8306 19.8144 15.3807 20.3424 15.82L20.8074 16.207C21.0814 16.435 21.3019 16.7204 21.4532 17.0431C21.6046 17.3658 21.6831 17.7179 21.6831 18.0743V18.0985C21.6831 19.4263 20.6049 20.5027 19.2748 20.5027V20.5028Z" fill="#FFDF55"/>
              <path d="M20.8059 16.207L20.3409 15.82C19.2716 14.9296 18.6533 13.6115 18.6533 12.2213V9.6478C18.6533 5.91993 15.6262 2.89771 11.8923 2.89771C11.2476 2.8973 10.6061 2.989 9.9873 3.17002C12.7937 3.99115 14.8433 6.58044 14.8433 9.64785V12.2213C14.8433 13.6114 15.4616 14.9296 16.531 15.82L16.9962 16.207C17.2702 16.435 17.4906 16.7204 17.6419 17.0431C17.7933 17.3658 17.8718 17.7178 17.8719 18.0742V18.0985C17.8719 19.4263 16.7937 20.5027 15.4635 20.5027H19.2735C20.6034 20.5027 21.6816 19.4263 21.6816 18.0985V18.0742C21.6816 17.7178 21.6031 17.3658 21.4517 17.0431C21.3004 16.7204 21.0799 16.435 20.8059 16.207Z" fill="#F7C92B"/>
              <circle cx="19" cy="5" r="4.5" fill="#F04438" stroke="white"/>
            </g>
            <defs>
              <clipPath id="clip0_816_35420">
                <rect width="24" height="24" fill="white"/>
              </clipPath>
            </defs>
          </svg>
        </div>
        <div class="flex gap-1 items-center relative transition-all duration-300 ease-in-out">
            <img @click="changeActive" src="@/assets/images/Avatar.png" alt="Avatar" class="h-10 w-10 max-md:h-8 max-md:w-8 cursor-pointer transition-all duration-300 ease-in-out">
            <svg class="max-lg:h-3 max-lg:w-3" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M4 6L7.29289 9.29289C7.62623 9.62623 7.79289 9.79289 8 9.79289C8.20711 9.79289 8.37377 9.62623 8.70711 9.29289L12 6" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
<!--          Content dropdown-->
          <div v-if="isActive" class="w-60 max-md:w-44 bg-white absolute top-14 right-0 z-10 shadow-sd-1 rounded-lg transition-all duration-300 ease-in-out">
            <div class="flex justify-between items-start py-3 px-6 gap-2 border-b border-color-7 mb-2 max-md:py-1 max-md:px-2">
              <img src="@/assets/images/Avatar.png" alt="Avatar" class="h-10 w-10 max-md:w-8 max-md:h-8">
              <div>
                <div class="text-sm font-semibold text-color-1 max-md:text-xs">Phạm Kim Chi</div>
                <div class="text-xs text-color-6 max-md:text-[10px]">kimchi2209@gmail.com</div>
              </div>
            </div>
            <div class="py-[4px] px-2 max-md:py-0">
              <div class="bg-color-5 p-2 flex gap-2 items-center rounded-tr-lg rounded-tl-lg border-b border-color-7 max-md:p-1">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M8.84767 3.46717C8.84767 5.047 7.58097 6.31378 6.00002 6.31378C4.41961 6.31378 3.15237 5.047 3.15237 3.46717C3.15237 1.88733 4.41961 0.621094 6.00002 0.621094C7.58097 0.621094 8.84767 1.88733 8.84767 3.46717ZM6 11.3791C3.66678 11.3791 1.69678 10.9999 1.69678 9.53676C1.69678 8.07311 3.67915 7.70733 6 7.70733C8.33376 7.70733 10.3032 8.08656 10.3032 9.54967C10.3032 11.0133 8.32085 11.3791 6 11.3791Z" fill="#7F56D9"/>
                </svg>
                <div class="text-sm font-medium text-color-4 max-md:text-xs">Quản trị viên</div>
              </div>
              <div class="bg-color-5 p-2 flex gap-2 items-center rounded-br-lg rounded-bl-lg mb-2 max-md:p-1">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g clip-path="url(#clip0_550_43615)">
                    <path d="M5.99805 0.625C6.5227 0.625 6.93595 1.02127 7.19975 1.55596L8.0808 3.33258C8.1075 3.38757 8.17085 3.46499 8.26605 3.53582C8.36115 3.60658 8.4543 3.64561 8.51555 3.6559L10.1103 3.92306C10.6864 4.01987 11.1693 4.30225 11.326 4.79395C11.4826 5.28525 11.2532 5.7958 10.8389 6.2108L10.8385 6.21125L9.59955 7.46045C9.55045 7.50995 9.49545 7.6032 9.46095 7.7247C9.4267 7.8454 9.42365 7.95535 9.4392 8.02635L9.4394 8.02735L9.79385 9.5727C9.9409 10.2158 9.89215 10.8537 9.43855 11.1871C8.98335 11.5217 8.36135 11.3734 7.79625 11.0368L6.3013 10.1445C6.2385 10.107 6.1307 10.0766 6.00055 10.0766C5.87135 10.0766 5.7613 10.1066 5.6944 10.1455L5.69345 10.1461L4.20144 11.0366C3.63702 11.3744 3.01577 11.52 2.56055 11.1851C2.10724 10.8516 2.05607 10.215 2.20355 9.57235L2.55795 8.02735L2.55816 8.02635C2.5737 7.95535 2.57066 7.8454 2.5364 7.7247C2.5019 7.6032 2.44689 7.50995 2.39779 7.46045L1.15792 6.2103C0.746324 5.7953 0.517604 5.2852 0.672974 4.79462C0.828794 4.30262 1.31071 4.0199 1.88716 3.92303L3.48066 3.6561L3.48116 3.65601C3.53958 3.64587 3.63135 3.60728 3.72624 3.53634C3.8213 3.46527 3.88479 3.38768 3.91156 3.33258L3.91291 3.32983L4.79281 1.55549L4.79316 1.55479C5.0595 1.02054 5.474 0.625 5.99805 0.625Z" fill="#7F56D9"/>
                  </g>
                  <defs>
                    <clipPath id="clip0_550_43615">
                      <rect width="12" height="12" fill="white"/>
                    </clipPath>
                  </defs>
                </svg>
                <div class="text-sm font-medium text-color-4 max-md:text-xs">Trưởng phòng MK</div>
              </div>
              <div class="border-b border-color-7">
                <div class="flex items-center gap-2 p-2 max-md:p-1">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_550_43622)">
                      <path d="M8.66683 1.36475C8.44756 1.34392 8.22516 1.33325 8.00016 1.33325C4.31868 1.33325 1.3335 4.18897 1.3335 7.71105C1.3335 9.40332 2.02238 10.9409 3.14609 12.082C3.3935 12.3333 3.55868 12.6765 3.49202 13.0298C3.382 13.6074 3.13266 14.1462 2.76757 14.5953C3.72813 14.7739 4.72692 14.6131 5.58349 14.1575C5.88628 13.9965 6.03768 13.916 6.14452 13.8997C6.25135 13.8833 6.40438 13.9121 6.71043 13.9696C7.13563 14.0494 7.5673 14.0895 8.00016 14.0889C11.2202 14.0889 14.0437 11.9043 14.6668 8.99992" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                      <path d="M10.9276 5.37782C10.5608 5.63055 9.59902 6.14658 10.1848 6.79235C10.4709 7.10775 10.7896 7.33335 11.1902 7.33335H13.4765C13.8771 7.33335 14.1958 7.10775 14.482 6.79235C15.0677 6.14658 14.106 5.63055 13.7392 5.37782C12.879 4.78519 11.7877 4.78519 10.9276 5.37782Z" stroke="#667085" stroke-width="1.5"/>
                      <path d="M13.4998 2.53325C13.4998 3.19599 12.9775 3.73325 12.3332 3.73325C11.6888 3.73325 11.1665 3.19599 11.1665 2.53325C11.1665 1.87051 11.6888 1.33325 12.3332 1.33325C12.9775 1.33325 13.4998 1.87051 13.4998 2.53325Z" stroke="#667085" stroke-width="1.5"/>
                      <path d="M7.99716 8H8.00316M5.3335 8H5.33948" stroke="#667085" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </g>
                    <defs>
                      <clipPath id="clip0_550_43622">
                        <rect width="16" height="16" fill="white"/>
                      </clipPath>
                    </defs>
                  </svg>
                  <div class="text-sm text-color-1 font-medium max-md:text-xs">Yêu cầu hỗ trợ</div>
                </div>
                <div class="flex items-center gap-2 p-2 max-md:p-1">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_550_43632)">
                      <path d="M9 13.3334C9 13.3334 9.66667 13.3334 10.3333 14.6667C10.3333 14.6667 12.451 11.3334 14.3333 10.6667" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                      <path d="M4.6665 10.6666H7.33317M4.6665 7.33325H9.99984" stroke="#667085" stroke-width="1.5" stroke-linecap="round"/>
                      <path d="M4.33334 2.33325C3.29614 2.36437 2.67774 2.47981 2.24985 2.9081C1.66406 3.49443 1.66406 4.43812 1.66406 6.32549V10.6629C1.66406 12.5503 1.66406 13.4939 2.24985 14.0803C2.83564 14.6666 3.77845 14.6666 5.66406 14.6666H7.33074M10.3281 2.33325C11.3653 2.36437 11.9837 2.47981 12.4116 2.90811C12.9974 3.49443 12.9974 4.43812 12.9974 6.32549V8.99992" stroke="#667085" stroke-width="1.5" stroke-linecap="round"/>
                      <path d="M4.33057 2.49992C4.33057 1.85559 4.85291 1.33325 5.49723 1.33325H9.16391C9.80824 1.33325 10.3306 1.85559 10.3306 2.49992C10.3306 3.14425 9.80824 3.66659 9.16391 3.66659H5.49723C4.85291 3.66659 4.33057 3.14425 4.33057 2.49992Z" stroke="#667085" stroke-width="1.5" stroke-linejoin="round"/>
                    </g>
                    <defs>
                      <clipPath id="clip0_550_43632">
                        <rect width="16" height="16" fill="white"/>
                      </clipPath>
                    </defs>
                  </svg>
                  <div class="text-sm text-color-1 font-medium max-md:text-xs">Nhắc nhở nhiệm vụ</div>
                </div>
                <div class="flex items-center gap-2 p-2 max-md:p-1">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M7.98979 10.2307C5.41138 10.2307 3.20947 10.6206 3.20947 12.1818C3.20947 13.7431 5.39741 14.1469 7.98979 14.1469C10.5682 14.1469 12.7695 13.7564 12.7695 12.1958C12.7695 10.6352 10.5822 10.2307 7.98979 10.2307Z" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M7.98993 8.00384C9.68199 8.00384 11.0534 6.63178 11.0534 4.93971C11.0534 3.24765 9.68199 1.87622 7.98993 1.87622C6.29786 1.87622 4.9258 3.24765 4.9258 4.93971C4.92008 6.62606 6.28262 7.99812 7.96834 8.00384H7.98993Z" stroke="#667085" stroke-width="1.42857" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <div class="text-sm text-color-1 font-medium">Cài đặt hồ sơ</div>
                </div>
                <div class="flex items-center gap-2 p-2 max-md:p-1">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8.00016 11.8983C11.7596 11.8983 13.4989 11.4161 13.6668 9.48026C13.6668 7.54578 12.4543 7.67018 12.4543 5.29666C12.4543 3.44268 10.697 1.33325 8.00016 1.33325C5.30334 1.33325 3.54606 3.44268 3.54606 5.29666C3.54606 7.67018 2.3335 7.54578 2.3335 9.48026C2.50213 11.4234 4.24135 11.8983 8.00016 11.8983Z" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M9.5929 13.9048C8.68348 14.9146 7.2648 14.9266 6.34668 13.9048" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <div class="text-sm text-color-1 font-medium max-md:text-xs">Thông báo</div>
                </div>
              </div>
              <div class="flex items-center gap-2 p-2 max-md:p-1">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9.66667 10.3333L13 7M13 7L9.66667 3.66667M13 7H5M5 1H4.2C3.0799 1 2.51984 1 2.09202 1.21799C1.7157 1.40973 1.40973 1.71569 1.21799 2.09202C1 2.51984 1 3.07989 1 4.2V9.8C1 10.9201 1 11.4802 1.21799 11.908C1.40973 12.2843 1.71569 12.5903 2.09202 12.782C2.51984 13 3.0799 13 4.2 13H5" stroke="#667085" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <div class="text-sm text-color-1 font-medium max-md:xs">Đăng xuất</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>

export default {
  data(){
    return{
      pageTitle : "",
      isActive : false
    }
  },
  props: {
    toggleSidebar: {
      type: Function,
      required: true
    },
    openSidebar:{
      type : Boolean,
      required : true
    }
  },
  methods: {
    goBack() {
      if (window.history.length > 1) {
        this.$router.back(); // Quay lại trang trước
      } else {
        this.$router.push("/"); // Nếu không có trang trước, quay về trang chủ
      }
    },
    changeActive(){
      this.isActive = !this.isActive
    }
  },
  watch: {
    $route(to) {
      this.pageTitle = to.meta.title;
    },
  },
  created() {
    this.pageTitle = this.$route.meta.title;
  },
}
</script>